	...
	for ($i=0; $i < count($dirs); $i++)
	{
		...
		if (($filename !== '.') && ($filename != '..'))
		{
			...
			if (is_dir($tdir.'/'.$filename))
			{
				...
				if($cls == "file") {
					$label = "<div style=\'line-height:9px; white-space: nowrap;\'><img src=config/image/icons/filetype/".$extension.".filetype.png class=directoryIcon /><span style=\'margin-top:7px;\'>".$filename."</span></div>";
				}
				else if($cls == "folder") {
					$label = $filename;
				}
