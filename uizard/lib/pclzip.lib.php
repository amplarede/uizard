...
*/
  ...
  function privDeleteByIndex($p_index, &$p_result_list)
  {
    ...
    for ($i=0, $j_start=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++)
    {
      ...
      if ($v_found)
      {
        PclTraceFctMessage(__FILE__, __LINE__, 2, "File '".$v_header_list['filename']."', index '$i' need to be deleted");
        unset($v_header_list[$v_nb_extracted]);
      }
      else
      {
        PclTraceFctMessage(__FILE__, __LINE__, 2, "File '".$v_header_list['filename']."', index '$i' will not be deleted");
        $v_nb_extracted++;
      }
    }
